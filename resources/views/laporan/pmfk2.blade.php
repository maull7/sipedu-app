                                    ...
                                    @isset($mapelList)
                                        @foreach ($mapelList as $m)
                                            <option value="{{ $m->id_pelajaran }}"
                                                {{ (string) request('mapel') === (string) $m->id_pelajaran ? 'selected' : '' }}>
                                                {{ $m->nama_mapel }}
                                            </option>
                                        @endforeach
                                    @endisset
                                    ...
                                    @isset($kelasList)
                                        @foreach ($kelasList as $k)
                                            <option value="{{ $k->id_kelas }}"
                                                {{ (string) request('kelas') === (string) $k->id_kelas ? 'selected' : '' }}>
                                                {{ $k->nama_kelas }}
                                            </option>
                                        @endforeach
                                    @endisset
                    ...
                    <div class="card-body">
                        @php
                            ...
                            if ($first) {
                                if (is_array($first)) {
                                    if (
                                        array_key_exists('kategori_penilaian', $first) &&
                                        array_key_exists('10%', $first)
                                    ) {
                                        $mode = 'index';
